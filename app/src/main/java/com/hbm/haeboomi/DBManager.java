...
public class DBManager {
	...
	private String[][] table = {
			...
			{"mac", "classno"},
			{"id", "pw", "pro_name"},
			{"id", "day", "time", "cname", "divide", "classno", "building", "5075"},
	...
	public String getData(int tableName) {
		...
		try {    //아래의 getList()를 실행
			return getList(g.execute(url).get(), tableName);    //g.execute(url).get() 수행시 JSON으로 파싱된 결과를 얻을 수 있다
	...
	public void putSchedule(final int day, final String time, final String cname, final String classno, final String divide) {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				String[] buildings = {"", "공학관 (성호관)", "대학본부.인문1관 (연암관)", "의학관",
						...
						"사회.경영1관 (다산관)", "일송아트홀", "산학관",
						"사회.경영2관 (담헌관)", "국제교육관 (국제관)", "국제회의관",
						"기초교육관 (퇴계관)", "일송기념도서관", "한림스포츠센터",
						"한림연구센터", "학군단", "제2산학관(예정)",
						"학생생활1관", "학생생활2관", "학생생활3관",
						"학생생활4관", "학생생활5관", "학생생활6관",
						"학생생활7관", "학생생활8관", "실내테니스장",
						"테니스장", "온실", "대운동장",
						"씨름장", "탁구장", "골프장",
						"다용도체육장", "인라인스케이트장", "한림대학교춘천성심병원", "정문"};    //index 0 ~ 33
				String building = null;
				String number = "";
				for(int i = 0; i < classno.length(); i++) {
					char c = classno.charAt(i);
					if(c == '-') break; //1163-7같은 경우 11637(5자리)가 되기 때문에 '-'가 나온다면 1163이 되도록 멈춰준다.
					number += Character.isDigit(c)? c : "";  //숫자만 골라준다.
				}
				switch(number.length()) {
					case 4:
						building = buildings[number.charAt(0) - '0'];
						break;
					case 5:
						building = buildings[Integer.parseInt(classno.substring(0, 2))];
						break;
				}
				String u = SERVER_ADDRESS + "pro_schedule_insert.php?id=" + id + "&cname=" + parsePHP(cname) + "&time=" + parsePHP(time)
						+ "&classno=" + classno + "&building=" + parsePHP(building) + "&day=" + day + "&gubun=0" + "&divide=" + divide; //1 : 75분수업, 0 : 50분수업
				if(SuccessFail(u).equals("success")) {
					u = SERVER_ADDRESS + "schedule_update.php?time=" + parsePHP(time) + "&day=" + day + "&divide=" + divide;
					SuccessFail(u);
				}
			}
		}).start();
	...
	public void putSchedule() {
		try {
			...
			String str = doc.toString().replaceAll("&nbsp;", "");   //ASCII 32번 띄어쓰기가 아닌 160번 띄어쓰기를 모두 삭제
			...
			u = SERVER_ADDRESS + "student_update.php?id=" + idpw[0] + "&name=" + array[1] + "&dept=" + array[3];
					...
					for(int j = 0; j < ary[i].length; j++) {
						...
						if(subject.length > 1) {    //공백이 아닐 때
							...
							if(i % 2 == 1)  //1, 3(2, 3, 5, 6, 8, 9, 11, 12교시)
								...
								if(i % 4 == 0)
									switch(j) {
										case 1: day = 1; break; //월
										case 2:
										case 3: day = 2; break; //화
										case 4: day = 3; break; //수
										case 5:
										case 6: day = 4; break; //목
										case 7: day = 5; break; //금
									}
								else {  //화요일, 목요일 75분수업
									if(j == 0) day = 2;         //화요일
									else if(j == 1) day = 4;    //목요일
								}
							}
							String time = null;
							switch(i % 4) {
								case 0:
									switch(i / 4) {
										case 0: time = "09:00:00"; break;
										case 1: time = "12:00:00"; break;
										case 2: time = "15:00:00"; break;
										case 3: time = "18:00:00"; break;
									}
									break;
								case 1:
									switch(i / 4) {
										case 0: time = "10:00:00"; break;
										case 1: time = "13:00:00"; break;
										case 2: time = "16:00:00"; break;
										case 3: time = "19:00:00"; break;
									}
									break;
								case 2:
									switch(i / 4) {
										case 0: time = "10:30:00"; break;
										case 1: time = "13:30:00"; break;
										case 2: time = "16:30:00"; break;
										case 3: time = "19:30:00"; break;
									}
									break;
								case 3:
									switch(i / 4) {
										case 0: time = "11:00:00"; break;
										case 1: time = "14:00:00"; break;
										case 2: time = "17:00:00"; break;
										case 3: time = "20:00:00"; break;
									}
									break;
							}
							boolean gubun = false;  //true : 75분 수업, false : 50분 수업
							switch(i % 4) {
	...
	public boolean delete(final String where, final int index) {
		String url = DBManager.SERVER_ADDRESS + "delete.php?index=" + index + "&where=" + parsePHP(where);
		if(SuccessFail(url).equals("success"))
			return true;
		else
			return false;
	}
	...
	public static class innerDB extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE if not exists user (id VARCHAR(8), password VARCHAR(15), PRIMARY KEY(id, password))");
		}
